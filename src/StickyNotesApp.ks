...
class StickyNotesApp {
    ...
    handleNoteFocusOut(event) {
        ...
        if (event.target.classList.contains("note-edit")) {
            ...
            // Get note id
            const id = parseInt(element.querySelector(".delete-btn").getAttribute("data-id"));
            
            // Edit note
            this.notesWall.editNote(id, event.target.value.trim());
    ...
    handleTextAreaKeyDown(event, isEditing = false) {
        ...
        if (event.key === "Enter" && !event.shiftKey) {
            ...
            if (isEditing) {
                // Focus out trigger
                event.target.blur();
            } else if (event.target.value.trim() !== "") { // Save new note
                // Add note
                this.notesWall.addNote(
...
// Issue 1: When entering multiline inputs, i.e. "ABC \n DEF", the textarea will display "ABC DEF" instead of "ABC \n DEF". However, when entering "ABC \n DEF \n GHI", the textarea will display "ABC \n DEF \n GHI", which works as expected.
// Issue 2: Pressing "Enter" or exiting text edit area doesn't save the note (change color and focus out) properly, however the text is updated.
// Issue 3: When text is too long, the textarea will expand beyond the note's boundary.
...
// 4. Spec: Notes should be displayed as sticky notes on a wall (a grid layout).
